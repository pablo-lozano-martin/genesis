...
async def save_to_history(
    ...
    try:
        ...
        logger.info(f"Saving {len(messages)} messages to conversation {conversation_id}")

        for message in messages:
            try:
                await message_repository.create(message)
                logger.debug(f"Saved message with role {message.role} to conversation {conversation_id}")
